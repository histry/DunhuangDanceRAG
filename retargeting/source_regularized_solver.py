...
def retarget_bvh(
    ...
    # Default-zero activation is a true baseline fast path: do not monkey-patch
    # the optimizer at all.  This makes the merged V2.4 mechanism behaviorally
    # identical to the historical retargeter until an experiment explicitly
    # enables one of the source-relative weights.
    if not settings.enabled:
        ...
        report["source_temporal_regularization"] = {
            **settings.to_dict(),
            ...
            "activation_contract": "explicit_environment_opt_in",
            "baseline_fast_path": True,
        }
    ...
    report["source_temporal_regularization"] = {
        **settings.to_dict(),
        "common_direct_mapped_bone_children": list(common_bones),
        "common_direct_mapped_bone_count": int(len(common_bones)),
        "activation_contract": "enabled_by_environment",
        "baseline_fast_path": False,
    }
    return motion, report
