...
SCHEMA = "refiner_observable_bridge_diagnostic_v1"
...
def build_banks(clean, cond, sources, cfg, device):
    banks = {}
    recipes = {}
            ...
            for index, original in enumerate(clean):
                ...
                for recipe_id, width in enumerate((10, 28)):
                    ...
                    rows.append({"window": index, "source": sources[index], "role": role,
                                 ...
                                 "a": a, "b": b, "hidden_clean_target": role == "single_recording"})
...
def evaluate(model, banks, split, cfg):
        ...
        for start in range(0, len(bank["clean"]), 8):
            ...
            for prediction, clean_prediction, reference, clean, seam in zip(*arrays):
                if role == "single_recording":
                    ...
                    details.append({"observable": physical["observable_boundary_gates"][-1],
                                    "clean_identity": physical["clean_identity_gates"][-1]})
                else:
                    gate = m._observable_boundary_audit(prediction, reference, seam, cfg)
                    safety = m.evaluate_stage_candidate(
                        m._safe_validation_audit(reference,cfg,role="cross_reference",support_policy="source_observation"),
                        m._safe_validation_audit(prediction,cfg,role="cross_prediction",support_policy="source_observation"),
                    ...
                    cross.append({"observable": gate, "safety": safety, "hidden_clean_used": False})
...
def run(args):
    ...
    for step in range(1,args.steps + 1):
        indices = rng.choice(len(train["clean"]),size=min(8,len(train["clean"])),replace=False)
        ...
        if logging:
            row = {"stage":"observable_bridge_fit","step":step,"target_steps":args.steps,
                   ...
                   "terms":{k:float(v.detach()) for k,v in terms.items()},"gradient":gradient,"clip_norm_before":norm}
    ...
    m.save_json(report,destination / "diagnostic_report.json")
    m._atomic_torch_save({"version":"observable_bridge_diagnostic_only_v1","formal_checkpoint":False,
