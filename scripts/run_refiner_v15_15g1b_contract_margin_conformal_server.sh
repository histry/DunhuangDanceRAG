#!/usr/bin/env bash
# Server-only V15.15g1b contract-margin/conformal correction probe.
...
TAG="refiner_v15_15g1b_contract_margin_conformal_${STAMP}"
ROOT="$OUT_ROOT/checkpoints/$TAG"
PROBE_DIR="$ROOT/contract_margin_conformal_probe"
LOG="logs/refiner_v15_15g1b_contract_margin_conformal_${STAMP}.log"
STATUS="outputs/refiner_v15_15g1b_contract_margin_conformal_${STAMP}.exit_status.txt"
SCIENTIFIC_STATUS="outputs/refiner_v15_15g1b_contract_margin_conformal_${STAMP}.scientific_status.txt"

mkdir -p "$ROOT" logs outputs
printf '%s\n' "$TAG" > outputs/LATEST_REFINER_V15_15G1B_TAG
printf '%s\n' "$LOG" > outputs/LATEST_REFINER_V15_15G1B_LOG
printf '%s\n' "$STATUS" > outputs/LATEST_REFINER_V15_15G1B_STATUS
printf '%s\n' "$SCIENTIFIC_STATUS" > outputs/LATEST_REFINER_V15_15G1B_SCIENTIFIC_STATUS
...
if [[ "$RUN_STATUS" -ne 0 && "$RUN_STATUS" -ne 2 ]]; then
  echo "[FATAL] V15.15g1b failed with status $RUN_STATUS"
  exit "$RUN_STATUS"
fi

REPORT="$PROBE_DIR/fixed_budget_correction.report.json"
ENVELOPE="$PROBE_DIR/transaction_conformal_severity_envelope.json"
test -s "$REPORT"
test -s "$ENVELOPE"
printf '%s\n' "$REPORT" > outputs/LATEST_REFINER_V15_15G1B_REPORT
printf '%s\n' "$ENVELOPE" > outputs/LATEST_REFINER_V15_15G1B_CONFORMAL_ENVELOPE
"$PY" - "$REPORT" <<'PY'
import json
import sys
...
print(json.dumps({
    ...
    "severity_conformal_threshold": a.get(
        "severity_conformal_threshold"
    ),
    "severity_activation_threshold": a.get(
...
PY
echo "V15.15g1b contract-margin/conformal probe completed; scientific_status=$RUN_STATUS"
