#!/usr/bin/env bash
# Server-only V15.14e group-local output-cone probe; never training.
set -Eeuo pipefail
cd "$(dirname "$0")/.."
...
TAG="refiner_v15_14e_group_local_nullspace_cone_probe_${STAMP}"
PROBE_DIR="$OUT_ROOT/checkpoints/$TAG/group_local_nullspace_cone_probe"
LOG="logs/refiner_v15_14e_group_local_nullspace_probe_${STAMP}.log"
STATUS="outputs/refiner_v15_14e_group_local_nullspace_probe_${STAMP}.exit_status.txt"
SCIENTIFIC_STATUS="outputs/refiner_v15_14e_group_local_nullspace_probe_${STAMP}.scientific_status.txt"

mkdir -p "$PROBE_DIR" logs outputs
printf '%s\n' "$TAG" > outputs/LATEST_REFINER_V15_14E_NULLSPACE_PROBE_TAG
printf '%s\n' "$LOG" > outputs/LATEST_REFINER_V15_14E_NULLSPACE_PROBE_LOG
printf '%s\n' "$STATUS" > outputs/LATEST_REFINER_V15_14E_NULLSPACE_PROBE_STATUS
printf '%s\n' "$SCIENTIFIC_STATUS" \
  > outputs/LATEST_REFINER_V15_14E_NULLSPACE_PROBE_SCIENTIFIC_STATUS
...
set +e
"$PY" -u -m training.refiner_group_local_nullspace_cone_probe \
...
if [[ "$PROBE_STATUS" -ne 0 && "$PROBE_STATUS" -ne 2 ]]; then
  echo "[FATAL] V15.14e probe execution failed with status $PROBE_STATUS"
  exit "$PROBE_STATUS"
fi

REPORT="$PROBE_DIR/group_local_nullspace_cone.report.json"
test -s "$REPORT"
"$PY" - "$REPORT" <<'PY'
import json
import sys
...
print(json.dumps({
    ...
    "local_action_shape": report.get("local_action_shape"),
    ...
    "nullspace_by_group": [
        {
            ...
            "nullspace_basis_dimension": row.get(
                "nullspace_basis_dimension"
            ),
            "jacobian_rank": row.get("jacobian_rank"),
            "nullspace_dimension": row.get("nullspace_dimension"),
        }
        for row in report.get("hard_constraint_fd_jacobian", [])
    ],
...
echo "V15.14e group-local nullspace cone completed; scientific_status=$PROBE_STATUS"
