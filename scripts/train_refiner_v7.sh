#!/usr/bin/env bash
# Isolated, foreground V7 workflow. Existing source and scheduler assets are read-only.
set -Eeuo pipefail
if [[ $# -ne 3 || ! "$1" =~ ^(diagnose|pilot|resume|generate)$ || ! "$3" =~ ^[a-zA-Z0-9_-]+$ ]]; then
  echo "Usage: bash scripts/train_refiner_v7.sh diagnose|pilot|resume|generate EXISTING_OUT_ROOT NEW_TAG" >&2
  exit 2
fi
MODE=$1
...
if [[ "$MODE" == diagnose ]]; then
  "$PY" -u -m training.refiner_bridge_diagnostics "${FIT_ARGS[@]}" \
    --out_dir "$FIT_DIR" --windows 8 --steps 400 --eval_every 200 \
