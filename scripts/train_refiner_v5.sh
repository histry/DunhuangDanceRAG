...
if [[ $# -ne 3 || ! "$1" =~ ^(pilot|resume)$ || ! "$3" =~ ^[a-zA-Z0-9_-]+$ ]]; then
  echo "Usage: bash scripts/train_refiner_v5.sh pilot|resume EXISTING_OUT_ROOT CANDIDATE_TAG" >&2
  exit 2
fi
MODE=$1
...
if pgrep -af '[t]raining/motion_models.py|[r]un_official_smpl_full.sh|[r]esearch_pipeline.sh|[s]cripts/pipeline.sh'; then
...
if [[ "$MODE" == pilot ]]; then
  if [[ -e "$SNAPSHOT" || -e "$REFINER" || -e "$CANDIDATES/boundary_refiner.best_validation.pt" ]]; then
    echo "[FATAL] Candidate already exists; use a new tag or resume the matching V5 snapshot" >&2
    exit 2
  fi
...
test -s "$SNAPSHOT" || { echo "[FATAL] V5 pilot snapshot missing" >&2; exit 2; }
