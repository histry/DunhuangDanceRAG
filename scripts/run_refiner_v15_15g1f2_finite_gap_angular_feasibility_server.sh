#!/usr/bin/env bash
# Server-only V15.15g1f2 train calibration.
...
TAG="refiner_v15_15g1f2_finite_gap_angular_feasibility_${STAMP}"
ROOT="$OUT_ROOT/checkpoints/$TAG"
TRAIN_PROBE="$ROOT/train_finite_gap_angular_feasibility_calibration"
LOG="logs/refiner_v15_15g1f2_finite_gap_angular_feasibility_${STAMP}.log"
STATUS="outputs/refiner_v15_15g1f2_finite_gap_angular_feasibility_${STAMP}.exit_status.txt"
SCIENTIFIC_STATUS="outputs/refiner_v15_15g1f2_finite_gap_angular_feasibility_${STAMP}.scientific_status.txt"

mkdir -p "$ROOT" logs outputs
printf '%s\n' "$TAG" > outputs/LATEST_REFINER_V15_15G1F2_TAG
printf '%s\n' "$LOG" > outputs/LATEST_REFINER_V15_15G1F2_LOG
printf '%s\n' "$STATUS" > outputs/LATEST_REFINER_V15_15G1F2_STATUS
printf '%s\n' "$SCIENTIFIC_STATUS" > outputs/LATEST_REFINER_V15_15G1F2_SCIENTIFIC_STATUS
...
if [[ "$TRAIN_STATUS" -ne 0 && "$TRAIN_STATUS" -ne 2 ]]; then
  echo "[FATAL] V15.15g1f2 train calibration failed with status $TRAIN_STATUS"
  exit "$TRAIN_STATUS"
fi
...
printf '%s\n' "$TRAIN_REPORT" > outputs/LATEST_REFINER_V15_15G1F2_TRAIN_REPORT
printf '%s\n' "$FROZEN_ENVELOPE" > outputs/LATEST_REFINER_V15_15G1F2_FROZEN_CONFORMAL
printf '%s\n' "$FROZEN_REPAIR" > outputs/LATEST_REFINER_V15_15G1F2_FROZEN_REPAIR_CONTRACT
...
summary = r.get("local_feasible_intersection_summary") or {}
assert r.get("schema") == (
    "refiner_v15_15g1f2_finite_gap_angular_feasibility_sqp_v1"
)
assert r.get("evaluation_role") == "train_calibration"
assert r.get("evaluation_split") == "train"
assert r.get("finite_gap_angular_feasibility_sqp") is True
assert r.get("per_angle_active_set") is True
assert r.get("second_order_hessian_used") is False
assert r.get("temporal_fd_float64_epsilon_ladder") == [
    1e-5, 3e-5, 1e-4, 3e-4
]
for uid, row in (summary.get("by_case_uid") or {}).items():
    if not uid.endswith(":131") or not row.get("requires_shadow_repair"):
        continue
    assert row.get("feasible_method_found") is False, uid
    assert any(
        method.get("step_rejection_reason")
        == "empty_geodesic_joint_feasible_intersection"
        and method.get("solver_failure_reason") == "zero_science_gradient"
        for method in (row.get("methods") or {}).values()
    ), uid
print(json.dumps({
    "stage": "v15_15g1f2_train_finite_gap_receipt",
    ...
    "temporal_fd_float64_epsilon_ladder": r.get(
        "temporal_fd_float64_epsilon_ladder"
    ),
    "finite_gap_angular_feasibility_sqp": r.get(
        "finite_gap_angular_feasibility_sqp"
    ),
    "per_angle_active_set": r.get("per_angle_active_set"),
    "second_order_hessian_used": r.get("second_order_hessian_used"),
    "local_feasible_intersection_summary": summary,
...
echo "V15.15g1f2 train-only finite-gap angular feasibility calibration completed; scientific_status=$TRAIN_STATUS"
