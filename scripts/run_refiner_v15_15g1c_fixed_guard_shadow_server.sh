#!/usr/bin/env bash
# Server-only V15.15g1c fixed-Guard-shadow activation/correction probe.
...
TAG="refiner_v15_15g1c_fixed_guard_shadow_${STAMP}"
ROOT="$OUT_ROOT/checkpoints/$TAG"
PROBE_DIR="$ROOT/fixed_guard_shadow_probe"
LOG="logs/refiner_v15_15g1c_fixed_guard_shadow_${STAMP}.log"
STATUS="outputs/refiner_v15_15g1c_fixed_guard_shadow_${STAMP}.exit_status.txt"
SCIENTIFIC_STATUS="outputs/refiner_v15_15g1c_fixed_guard_shadow_${STAMP}.scientific_status.txt"

mkdir -p "$ROOT" logs outputs
printf '%s\n' "$TAG" > outputs/LATEST_REFINER_V15_15G1C_TAG
printf '%s\n' "$LOG" > outputs/LATEST_REFINER_V15_15G1C_LOG
printf '%s\n' "$STATUS" > outputs/LATEST_REFINER_V15_15G1C_STATUS
printf '%s\n' "$SCIENTIFIC_STATUS" > outputs/LATEST_REFINER_V15_15G1C_SCIENTIFIC_STATUS
...
if [[ "$RUN_STATUS" -ne 0 && "$RUN_STATUS" -ne 2 ]]; then
  echo "[FATAL] V15.15g1c failed with status $RUN_STATUS"
  exit "$RUN_STATUS"
fi
...
test -s "$REPORT"
test -s "$ENVELOPE"
printf '%s\n' "$REPORT" > outputs/LATEST_REFINER_V15_15G1C_REPORT
printf '%s\n' "$ENVELOPE" > outputs/LATEST_REFINER_V15_15G1C_DISCRIMINATIVE_CONFORMAL
"$PY" - "$REPORT" <<'PY'
import json
import sys
...
PY
echo "V15.15g1c fixed-Guard-shadow probe completed; scientific_status=$RUN_STATUS"
