#!/usr/bin/env bash
# Server-only V15.14f group-local hard-inequality probe; never training.
set -Eeuo pipefail
cd "$(dirname "$0")/.."
...
TAG="refiner_v15_14f_group_local_hard_inequality_cone_probe_${STAMP}"
PROBE_DIR="$OUT_ROOT/checkpoints/$TAG/group_local_inequality_cone_probe"
LOG="logs/refiner_v15_14f_group_local_inequality_probe_${STAMP}.log"
STATUS="outputs/refiner_v15_14f_group_local_inequality_probe_${STAMP}.exit_status.txt"
SCIENTIFIC_STATUS="outputs/refiner_v15_14f_group_local_inequality_probe_${STAMP}.scientific_status.txt"

mkdir -p "$PROBE_DIR" logs outputs
printf '%s\n' "$TAG" > outputs/LATEST_REFINER_V15_14F_INEQUALITY_PROBE_TAG
printf '%s\n' "$LOG" > outputs/LATEST_REFINER_V15_14F_INEQUALITY_PROBE_LOG
printf '%s\n' "$STATUS" > outputs/LATEST_REFINER_V15_14F_INEQUALITY_PROBE_STATUS
printf '%s\n' "$SCIENTIFIC_STATUS" \
  > outputs/LATEST_REFINER_V15_14F_INEQUALITY_PROBE_SCIENTIFIC_STATUS
...
set +e
"$PY" -u -m training.refiner_group_local_inequality_cone_probe \
...
if [[ "$PROBE_STATUS" -ne 0 && "$PROBE_STATUS" -ne 2 ]]; then
  echo "[FATAL] V15.14f probe execution failed with status $PROBE_STATUS"
  exit "$PROBE_STATUS"
fi

REPORT="$PROBE_DIR/group_local_inequality_cone.report.json"
test -s "$REPORT"
"$PY" - "$REPORT" <<'PY'
import json
import sys
...
print(json.dumps({
    ...
    "solver_failure_counts": report.get("solver_failure_counts"),
    "linear_feasible_direction_by_group": report.get(
        "linear_feasible_direction_by_group"
    ),
    "resolved_candidate_audit_by_group": report.get(
        "resolved_candidate_audit_by_group"
    ),
    "raw_exact_common_descent_by_group": report.get(
        "raw_exact_common_descent_by_group"
    ),
    ...
    "outside_group_or_ownership_abs_max": report.get(
...
echo "V15.14f group-local hard-inequality cone completed; scientific_status=$PROBE_STATUS"
