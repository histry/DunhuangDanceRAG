#!/usr/bin/env bash
# Server-only validation and a fresh diagnostic; never pilot or promotion.
set -Eeuo pipefail
cd "$(dirname "$0")/.."
...
TAG="refiner_v15_8_world_dynamics_${STAMP}"
mkdir -p outputs logs
LOG="logs/refiner_v15_8_${STAMP}.log"
STATUS="outputs/refiner_v15_8_${STAMP}.exit_status.txt"
printf '%s\n' "$TAG" > outputs/LATEST_REFINER_V15_8_TAG
printf '%s\n' "$LOG" > outputs/LATEST_REFINER_V15_8_LOG
printf '%s\n' "$STATUS" > outputs/LATEST_REFINER_V15_8_STATUS
