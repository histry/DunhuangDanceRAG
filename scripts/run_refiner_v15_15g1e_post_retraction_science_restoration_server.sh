#!/usr/bin/env bash
# Server-only V15.15g1e train calibration and reused-development probe.
...
TAG="refiner_v15_15g1e_post_retraction_science_restoration_${STAMP}"
...
LOG="logs/refiner_v15_15g1e_post_retraction_science_restoration_${STAMP}.log"
STATUS="outputs/refiner_v15_15g1e_post_retraction_science_restoration_${STAMP}.exit_status.txt"
SCIENTIFIC_STATUS="outputs/refiner_v15_15g1e_post_retraction_science_restoration_${STAMP}.scientific_status.txt"

mkdir -p "$ROOT" logs outputs
printf '%s\n' "$TAG" > outputs/LATEST_REFINER_V15_15G1E_TAG
printf '%s\n' "$LOG" > outputs/LATEST_REFINER_V15_15G1E_LOG
printf '%s\n' "$STATUS" > outputs/LATEST_REFINER_V15_15G1E_STATUS
printf '%s\n' "$SCIENTIFIC_STATUS" > outputs/LATEST_REFINER_V15_15G1E_SCIENTIFIC_STATUS
...
if [[ "$TRAIN_STATUS" -ne 0 && "$TRAIN_STATUS" -ne 2 ]]; then
  echo "[FATAL] V15.15g1e train calibration failed with status $TRAIN_STATUS"
  exit "$TRAIN_STATUS"
fi
...
printf '%s\n' "$TRAIN_REPORT" > outputs/LATEST_REFINER_V15_15G1E_TRAIN_REPORT
printf '%s\n' "$FROZEN_ENVELOPE" > outputs/LATEST_REFINER_V15_15G1E_FROZEN_CONFORMAL
printf '%s\n' "$FROZEN_REPAIR" > outputs/LATEST_REFINER_V15_15G1E_FROZEN_REPAIR_CONTRACT
...
print(json.dumps({
    "stage": "v15_15g1e_train_feasible_intersection_rejected",
    "scientific_status": 2,
...
if [[ "$DEV_STATUS" -ne 0 && "$DEV_STATUS" -ne 2 ]]; then
  echo "[FATAL] V15.15g1e development validation failed with status $DEV_STATUS"
  exit "$DEV_STATUS"
fi

DEV_REPORT="$DEV_PROBE/fixed_budget_correction.report.json"
test -s "$DEV_REPORT"
printf '%s\n' "$DEV_REPORT" > outputs/LATEST_REFINER_V15_15G1E_DEVELOPMENT_REPORT
...
PY
echo "V15.15g1e train calibration and reused-development probe completed; scientific_status=$DEV_STATUS"
