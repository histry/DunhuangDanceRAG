#!/usr/bin/env bash
# Foundation + current V12 full-cycle local Refiner. Keep this entrypoint for existing runbooks.
...
if [[ "$MODE" == diagnose ]]; then
  ...
  echo "[OPTIMIZER] One equal-weight 192-case TRAIN transaction with total+endpoint+raw-temporal subgroup guard + Pareto rescue: 32 seen + 5x32 context; no probe fitting."
