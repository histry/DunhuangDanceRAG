#!/usr/bin/env bash
# Server-only 50-step V15.12f learnability probe; never pilot or promotion.
set -Eeuo pipefail
cd "$(dirname "$0")/.."
...
TAG="refiner_v15_12f_exact_guard_constrained_mgda_probe_${STAMP}"
CANDIDATES="$OUT_ROOT/checkpoints/$TAG"
LOG="logs/refiner_v15_12f_probe_${STAMP}.log"
STATUS="outputs/refiner_v15_12f_probe_${STAMP}.exit_status.txt"
SCIENTIFIC_STATUS="outputs/refiner_v15_12f_probe_${STAMP}.diagnostic_status.txt"

mkdir -p logs outputs
printf '%s\n' "$TAG" > outputs/LATEST_REFINER_V15_12F_PROBE_TAG
printf '%s\n' "$LOG" > outputs/LATEST_REFINER_V15_12F_PROBE_LOG
printf '%s\n' "$STATUS" > outputs/LATEST_REFINER_V15_12F_PROBE_STATUS
printf '%s\n' "$SCIENTIFIC_STATUS" \
  > outputs/LATEST_REFINER_V15_12F_PROBE_DIAGNOSTIC_STATUS
...
if [[ "$DIAG_STATUS" -ne 0 && "$DIAG_STATUS" -ne 2 ]]; then
  echo "[FATAL] V15.12f probe execution failed with status $DIAG_STATUS"
  exit "$DIAG_STATUS"
fi
...
echo "V15.12f 50-step probe completed; diagnostic_status=$DIAG_STATUS"
