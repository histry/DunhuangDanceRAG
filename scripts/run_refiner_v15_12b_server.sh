#!/usr/bin/env bash
# Server-only validation and fresh V15.12b diagnostic; never pilot or promotion.
set -Eeuo pipefail
cd "$(dirname "$0")/.."
...
TAG="refiner_v15_12b_fixed_guard_${STAMP}"
mkdir -p outputs logs
LOG="logs/refiner_v15_12b_${STAMP}.log"
STATUS="outputs/refiner_v15_12b_${STAMP}.exit_status.txt"
printf '%s\n' "$TAG" > outputs/LATEST_REFINER_V15_12B_TAG
printf '%s\n' "$LOG" > outputs/LATEST_REFINER_V15_12B_LOG
printf '%s\n' "$STATUS" > outputs/LATEST_REFINER_V15_12B_STATUS
...
echo "V15.12b DIAGNOSTIC PASSED; no pilot, training resume, phase model, or promotion was launched."
