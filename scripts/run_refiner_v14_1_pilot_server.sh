#!/usr/bin/env bash
# Fresh V14.1 2,000-step pilot. Never resumes V14, publishes, or generates.
set -Eeuo pipefail
...
EXPECTED_COMMIT="${EXPECTED_COMMIT:?Set EXPECTED_COMMIT to the reviewed V14.1 commit}"
RUN_TAG="${V14_1_RUN_TAG:-$(date +%Y%m%d_%H%M%S)}"
TAG="refiner_v14_1_normalized_guard_pilot_${RUN_TAG}"
...
if [[ -e "$RUN_ROOT" ]]; then
  echo "[FATAL] V14.1 pilot output already exists: $RUN_ROOT" >&2
  exit 2
fi
...
printf '%s\n' "$RUN_ROOT" > outputs/LATEST_REFINER_V14_1_PILOT_ROOT
...
print(json.dumps({
    "stage": "refiner_v14_1_normalized_guard_pilot_paused",
...
echo "[NO PROMOTION] V14.1 pilot did not publish a Refiner or start generation."
