#!/usr/bin/env bash
# Server-only 100-step V15.15d case-isolated fixed-Guard restoration probe.
set -Eeuo pipefail
cd "$(dirname "$0")/.."
...
SOURCE_ADAPTER_TAG="${SOURCE_ADAPTER_TAG:-$(cat outputs/LATEST_REFINER_V15_15C_EXACT_RADIUS_PROBE_TAG)}"
...
TAG="refiner_v15_15d_case_isolated_guard_probe_${STAMP}"
ROOT="$OUT_ROOT/checkpoints/$TAG"
PROBE_DIR="$ROOT/adapter_probe"
LOG="logs/refiner_v15_15d_case_isolated_guard_probe_${STAMP}.log"
STATUS="outputs/refiner_v15_15d_case_isolated_guard_probe_${STAMP}.exit_status.txt"
SCIENTIFIC_STATUS="outputs/refiner_v15_15d_case_isolated_guard_probe_${STAMP}.scientific_status.txt"

mkdir -p "$PROBE_DIR" logs outputs
printf '%s\n' "$TAG" > outputs/LATEST_REFINER_V15_15D_GUARD_PROBE_TAG
printf '%s\n' "$LOG" > outputs/LATEST_REFINER_V15_15D_GUARD_PROBE_LOG
printf '%s\n' "$STATUS" > outputs/LATEST_REFINER_V15_15D_GUARD_PROBE_STATUS
printf '%s\n' "$SCIENTIFIC_STATUS" > outputs/LATEST_REFINER_V15_15D_GUARD_PROBE_SCIENTIFIC_STATUS
...
if [[ "$PROBE_STATUS" -ne 0 && "$PROBE_STATUS" -ne 2 ]]; then
  echo "[FATAL] V15.15d Adapter probe failed with status $PROBE_STATUS"
  exit "$PROBE_STATUS"
fi
...
print(json.dumps({
    "schema": r.get("schema"),
    "steps": r.get("steps"),
    "case_isolated_fixed_guard_restoration": r.get(
        "case_isolated_fixed_guard_restoration"
    ),
    "fixed_guard_training_aggregation": r.get(
        "fixed_guard_training_aggregation"
    ),
    "fixed_guard_training_scale": r.get("fixed_guard_training_scale"),
    "guard_safety_fraction": r.get("guard_safety_fraction"),
    "cross_group_balanced_training": r.get(
        "cross_group_balanced_training"
    ),
...
echo "V15.15d case-isolated fixed-Guard probe completed; scientific_status=$PROBE_STATUS"
