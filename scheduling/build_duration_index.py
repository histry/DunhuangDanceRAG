#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Augment a Music Router shared Event-RAG index with natural-duration priors.

Dynamic-event source length is used for non-turn events.  For turn-bearing
events, Duration Model Stage 1 supplies an inference-safe calibrated natural duration.
"""
...
def estimate_one(
    ...
    # The index event is already a real natural action.  Duration Model is a calibration
...
def main() -> None:
    ...
    parser.add_argument("--duration_model_checkpoint", required=True)
    ...
    for index, item in enumerate(items):
        ...
        if (index + 1) % 100 == 0 or index + 1 == len(items):
            print(f"[Whole-Song Planner duration index] {index + 1}/{len(items)}", flush=True)
    ...
    out_arrays.update(
        {
            ...
            "duration_model_duration_used": np.asarray([r["duration_model_used"] for r in rows], dtype=np.bool_),
    ...
    payload = {
        ...
        "duration_model_checkpoint": str(args.duration_model_checkpoint),
        "num_events": len(rows),
        "duration_model_used_count": int(sum(int(r["duration_model_used"]) for r in rows)),
