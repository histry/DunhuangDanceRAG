...
class MixedBuildPipelineTests(unittest.TestCase):
    def test_train_split_is_embedded_immediately_after_mixed_training(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            environment = {
                "GROUNDING_GROUNDER_ENABLE": "1",
                "GROUNDING_TRAIN_GROUNDER_ON_BUILD": "1",
                "GROUNDING_GROUNDER_ARCHITECTURE": "mixed",
                "GROUNDING_GROUNDER_PAIRED_DATASET": str(paired),
                "GROUNDING_GROUNDER_CKPT": str(checkpoint),
            }
            with patch.dict(os.environ, environment, clear=False), patch(
                "events.build_pipeline.anatomy_pipeline_runtime.main", return_value=0
            ), patch(
            ...
            report = json.loads(
                (out_dir / "events.motion_53.build.json").read_text(
                    encoding="utf-8"
                )
            )
