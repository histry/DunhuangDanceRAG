#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Geometry-Aware Routing Event-DB builder.

Execution order:
1. run the public Anatomy-Heading anatomy-gated builder;
...
The file is deliberately a small wrapper so the preserved Fresh-Audio Generation slicer and all
legacy Contrastive Retriever/Motion Refiner and Motion Diffusion NPZ keys remain unchanged.
"""
...
from events import anatomy_pipeline as anatomy_pipeline_runtime
...
def _checkpoint_for(out_dir: Path) -> Path:
    explicit = str(os.environ.get("GROUNDING_GROUNDER_CKPT", "")).strip()
    if explicit:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    rc = int(anatomy_pipeline_runtime.main(args))
    if rc != 0:
        return rc

    out_raw = _value(args, "--out_db")
    if not out_raw:
        raise RuntimeError("Geometry-Aware Routing event builder requires --out_db")
    ...
    geometry_report = augment_database(
        db_path,
        out_dir / "events.event_geometry_geometry.audit.json",
    ...
    if architecture not in {"legacy", "mixed"}:
        raise RuntimeError(
            "GROUNDING_GROUNDER_ARCHITECTURE must be 'legacy' or 'mixed', "
    ...
    if _env_bool("GROUNDING_GROUNDER_ENABLE", True):
        if split == "train" and _env_bool("GROUNDING_TRAIN_GROUNDER_ON_BUILD", True):
            if architecture == "mixed":
                paired_path = str(
                    os.environ.get("GROUNDING_GROUNDER_PAIRED_DATASET", "")
                ).strip()
                if not paired_path:
                    raise RuntimeError(
                        "Mixed grounding requires "
                        "GROUNDING_GROUNDER_PAIRED_DATASET; the builder will not "
                ...
                training_report = train_mixed_grounder(
                    ...
                    out_path=ckpt,
                    epochs=_env_int("GROUNDING_MIXED_EPOCHS", 120),
                    batch_size=_env_int("GROUNDING_GROUNDER_BATCH", 96),
                    seed=_env_int("GROUNDING_SEED", 20260724),
                )
                embedding_report = embed_database_mixed(db_path, ckpt)
                grounding_report = {
                    "schema": "event_geometry_mixed_train_and_embed_v1",
                    "ok": bool(
                ...
                grounding_report = train_grounder(
                    db_path=db_path,
                    out_path=ckpt,
                    steps=_env_int("GROUNDING_GROUNDER_STEPS", 1400),
                    batch_size=_env_int("GROUNDING_GROUNDER_BATCH", 128),
                    seed=_env_int("GROUNDING_SEED", 20260717),
                )
        elif ckpt.is_file():
            if architecture == "mixed":
    ...
    report = {
        "schema": "event_geometry_hierarchical_intrinsic_event_db",
        "split": split,
        "grounder_architecture": architecture,
        "out_dir": str(out_dir),
        "geometry": geometry_report,
        "grounding": grounding_report,
        "environment": {k: v for k, v in os.environ.items() if k.startswith("GROUNDING_")},
        "ok": True,
    }
    (out_dir / "events.motion_53.build.json").write_text(
